...
enum ClefInputMode { microphone, buttons }
...
class TrainingViewModel extends BaseViewModel {
  ...
  void toggleInputMode(ClefInputMode mode) {
    _inputMode = mode;
    if (mode == ClefInputMode.microphone) {
      startListening();
    } else {
  ...
  void initialize() async {
    ...
    spawnInitialNotes();
    _isRunning = true;
    if (_inputMode == ClefInputMode.microphone) {
      startListening();
    }
    notifyListeners();
  }
  ...
  void tick() {
    ...
    if (active != null && active.xFraction <= hitZone) return;
  ...
  void validateMidi(int detectedMidi) {
    ...
    if (PitchConverter.notesMatch(detectedMidi, expected)) {
      markCorrect(active, reactionTime);
    } else {
  ...
  void markCorrect(ScrollingNote sn, int reactionTime) {
    sn.answered = true;
    sn.correct = true;
  ...
  void markIncorrect(ScrollingNote sn, int reactionTime) {
    ...
    correctAnswer = enharmonicLabel(sn.note.noteName);
    lastAnswerFeedback =
        '✗ $lastPressed → ${enharmonicLabel(sn.note.noteName)}';
    clearFeedbackAfterDelay();
    attempts.add({
      'pressed': lastPressed,
      'expected': enharmonicLabel(sn.note.noteName),
  ...
  void playNoteSound(String noteLetter) {
    final midi = config.noteToMidiMap[noteLetter];
    if (midi == null) return;
